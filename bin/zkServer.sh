...
ZOOBIN="${BASH_SOURCE-$0}" #文件当前路径
ZOOBIN="$(dirname "${ZOOBIN}")" #文件目录，可能是相对的路径
ZOOBINDIR="$(cd "${ZOOBIN}"; pwd)" #文件目录，绝对的路径

if [ -e "$ZOOBIN/../libexec/zkEnv.sh" ]; then
  . "$ZOOBINDIR"/../libexec/zkEnv.sh
else
  . "$ZOOBINDIR"/zkEnv.sh #zk根目录下没有libexec文件，执行zkEnv.sh
fi
...
ZOO_DATADIR="$($GREP "^[[:space:]]*dataDir" "$ZOOCFG" | sed -e 's/.*=//')" #从zoo.cfg文件中找出dataDir路径
...
start)
    ...
    if [ -f "$ZOOPIDFILE" ]; then
      #kill -0用来做系统存活测试，存活返回true，不存活返回false
      if kill -0 `cat "$ZOOPIDFILE"` > /dev/null 2>&1; then #pid存在返回0，不存在返回1
